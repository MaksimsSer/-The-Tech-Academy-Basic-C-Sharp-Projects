...
namespace StringsAndIntegers
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<int> intList = new List<int>() { 5, 99, 23, 10, 185, 22, 12, 2994, 101, 770000};
            ...
            while (!numValid)
            {
                ...
                try
                {
                    Console.WriteLine("\nEnter a number to divide the abovw list by: ");
                    float numDivide = float.Parse(Console.ReadLine());
                    //If statmenet to check if the user entered 0
                    if (numDivide == 0)
                    {
